...
int main(int argc, char* argv[]){
    ...
    if(!args){
        ...
        if(read_file(n_file)){
            int vsol,best_long=9999999;
            if(voraz2(0,0,vsol,tmp)){
                best_sol=tmp;
                best_long=vsol;
            }
            auto start= clock();
            backtracking3(0,0,0,best_long,current,best_sol);
            auto end=clock();
            if(best_sol.size()>0){
                cout<<s1<<best_long<<endl;
       ...
       //cout<<i_lab[x][y]<<endl;
        if(laberinto[x][y]==1&&b_lab[x][y]==false&&recorrido<i_lab[x][y]&&recorrido+1<mejor_recorrido){//accessible,visitat,recorregut casella menor, recorregut menor que millor recorregut
            ...
            if(x==row-1&&y==col-1){//fulla
                ...
                if(recorrido+chebyshov(x,y)<mejor_recorrido){//prometer
